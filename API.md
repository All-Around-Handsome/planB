...
> 횟수 차감은 작업 시작 전에 호출하는 **차감 API(4번 항목)** 에서 이루어집니다.
> 전체 흐름: `남은 횟수 조회 → 버튼 클릭 시 차감 → AI 호출 → 저장`
>
> **참고**: AI 작업(9칸 채우기, 점수·리스크·액션 산출)은 프론트↔AI에서 이루어지고, 백엔드 저장 API는 **완성된 결과를 받아 저장만** 합니다.
...
AI가 생성한 BMC 9개 항목을 저장합니다. (점수·리스크·액션 없이 9칸만 저장)
...
직접 분석 결과를 저장합니다. 9개 항목에 더해 **타당성 점수, 리스크, 액션**까지 저장합니다. (점수·리스크·액션은 AI가 산출한 결과이며, 백엔드는 이를 받아 저장만 합니다.) `bmcType`은 백엔드가 `DIRECT_ANALYSIS`로 고정하므로 보내지 않아도 됩니다.
...
(수정할 `itemId`는 상세 조회(5-2) 응답의 각 항목에 포함되어 있습니다.)
...
> 하루 사용 횟수는 **생성 / 분석 / 탐색**을 각각 별개로 관리합니다. (각 5회)
> - **남은 횟수만 보고 싶을 때** → 4-1 조회 API (차감 없음)
> - **실제로 작업을 시작할 때** → 4-2 / 4-3 / 4-4 차감 API (호출 시 1 차감)
>
> **권장 프론트 흐름**
> ```
> ① 페이지 진입 → GET /api/bmc/limit (남은 횟수 표시, 차감 X)
> ② 버튼 클릭 → 해당 차감 API 호출 (여기서 차감)
...
오늘 남은 생성/분석/탐색 횟수를 반환합니다. **횟수를 차감하지 않습니다.** 페이지 진입 시 "생성 3/5, 분석 4/5, 탐색 5/5" 표시용.
...
> `remaining*`: 각각 오늘 남은 횟수 / `*Limit`: 각각 하루 한도
...
직접 분석을 시작하기 직전(분석 버튼 클릭 시)에 호출합니다. 분석 횟수가 1 차감됩니다.
...
|------|------|
| 메서드 | `POST` |
| 주소 | `/api/bmc/check-search` |
| 인증 | **필요** |

경쟁 서비스 탐색(S-001, S-002)을 시작하기 직전(탐색 버튼 클릭 시)에 호출합니다. 탐색 횟수가 1 차감됩니다.

> **S-001과 S-002는 하나의 탐색 카운트를 공유합니다.** (둘을 합쳐 하루 5회)
...
> **세 카운트는 서로 독립적입니다.** 생성을 5회 모두 사용해도 분석·탐색 횟수는 영향받지 않습니다.
> 모든 카운트는 **날짜 기준으로 자정에 자동 초기화**됩니다.
...
| 탐색 횟수 차감 | POST | `/api/bmc/check-search` | ✅ |
...
3. **AI 작업 흐름**: 남은 횟수 조회(`GET /api/bmc/limit`) → 버튼 클릭 시 차감(`check-generation` / `check-analysis` / `check-search`) → `canProceed: true`면 AI 호출 → (BMC의 경우) 저장 API로 저장. 횟수는 차감 API에서만 줄어듭니다.
